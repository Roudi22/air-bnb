...
// book place endpoint
userRouter.post("/book-place", verifyToken ,async (req, res) => {
  const { placeId, nights } = req.body;
  ...
  const bookingExist = await BookingModel.findOne({ place: placeId, user: req.user.id });
  if (bookingExist) {
    return res.status(400).json({
      message: "Place already booked",
    });
  }
  const booking = await BookingModel.create({ place: placeId, nights, user: req.user.id});
  await booking.save();
  const place = await PlaceModel.findById(placeId);
  if (!place.bookings) {
    place.bookings = [];
    place.bookings.push(req.user.id);
    await place.save();
    res.json({
...
// get all user bookings endpoint
userRouter.get("/bookings", verifyToken, async (req, res) => {
  const bookings = await BookingModel.find({ user: req.user.id });
  console.log("usre bookings",bookings);
  const bookedPlacesPromises = bookings.map(async (booking) => {
    const place = await PlaceModel.findById(booking.place);
    return { place };
  });
  
  const bookedPlaces = await Promise.all(bookedPlacesPromises);
  
  res.status(200).json({
    message: "User bookings retrieved successfully",
    bookings: bookedPlaces,
  });
});